...
static func from_dict(
	_source_kind: AbilityDefs.SourceKind,
	_source_id: String,
	rule: Dictionary,
	_can_be_doubled: bool = true
) -> PendingRule:
	var pending := PendingRule.new()
	pending.source_kind = _source_kind
	pending.source_id = _source_id
	...
	pending.can_be_doubled = _can_be_doubled
	return pending
